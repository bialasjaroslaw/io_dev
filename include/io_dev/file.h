...
namespace IO{
    class File: public Device{
        public:
            ...
            std::optional<uint64_t> read_bytes(uint64_t amount = ALL, char* buff = nullptr, uint64_t buff_size = 0) override;
            std::optional<uint64_t> pos() const override;
            std::optional<uint64_t> size() const override;
            std::optional<uint64_t> seek(uint64_t offset, SeekMode mode = SeekMode::Jump) override;
