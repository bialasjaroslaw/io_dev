...
namespace IO {
enum class OpenMode { Unknown, Read, Write, ReadBinary, WriteBinary };

enum class SeekMode { Jump, Skip, End };
...
class Device {
 public:
  ...
  virtual std::optional<uint64_t> read_bytes(uint64_t amount = ALL,
                                             char* buff = nullptr,
                                             uint64_t buff_size = 0) = 0;
  bool can_read() const {
  ...
  std::optional<std::string> read(uint64_t amount = ALL) {
    ...
    if (!bytes_read) return std::nullopt;
  ...
  std::optional<uint64_t> available() const {
    auto p = pos();
    auto s = size();
    if (!p || !s || p.value() > s.value()) return std::nullopt;
    return std::max(uint64_t{0}, s.value() - p.value());
  }
  virtual std::optional<uint64_t> seek(uint64_t offset,
                                       SeekMode mode = SeekMode::Jump) = 0;
